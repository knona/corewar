...
int get_all_args(t_asm *parse, char *line, t_instr tmp)
{
	...
	while (++n < MAX_ARGS_NUMBER)
		...
		{
			return (ft_super_free(4, tmp.command, tmp.args[0], tmp.args[1], tmp.args[2]));
		}
	...
	{
		ft_super_free(4, tmp.command, tmp.args[0], tmp.args[1], tmp.args[2]);
		return (!ft_dprintf(2, "Error line %d : too many arguments\n", tmp.line));
	}
...
int get_command(t_asm *parse, char *line)
{
	...
	{
		return (!ft_dprintf(2,
							"The command line %d begins with a f*cking coma "
							"biatch 🤦‍♀️ 🤦\n",
							parse->line));
	}
	if (!line[0])
