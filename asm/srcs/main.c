...
int get_flags_and_file(t_asm *parse, int ac, char **av)
{
	...
	while (++n < ac)
		if (!parse->file_name)
		{
			if (av[n][ft_strlen(av[n]) - 1] == 's' && av[n][ft_strlen(av[n]) - 2] == '.')
			{
...
void print_info(t_asm *parse)
{
	...
	while (parse->instrs[++n].command)
		ft_printf("%-15s%-15s%-15s%s\n",
				  ...
				  parse->instrs[n].args[0],
				  parse->instrs[n].args[1],
...
int main(int ac, char **av)
{
	...
	if (!get_comment_and_name(parse, fd) || !get_command_and_label(parse, fd))
	{
		get_next_line(fd, 0);
		close(fd);
		return (free_asm(parse));
