...
int get_size_instrs(t_asm *sam)
{
	...
	if (!(sam->instrs_asm = (t_instr_asm *)malloc(sizeof(t_instr_asm) * sam->nb_instrs)))
		return (0);
	i = 0;
	while (i < sam->nb_instrs)
	{
		...
		sam->instrs_asm[i].ocp = get_ocp(sam->instrs[i].args, sam->instrs_asm[i].op_code);
		j = 0;
		while (j < sam->instrs[i].nb_arg)
		{
			size_arg(sam->instrs[i].args[j], &sam->instrs_asm[i].op_args[j], selected_op);
			j++;
		}
		i++;
	}
