...
int get_champ(char *filename, t_champ_decoded *champ, int id, int visu_id)
{
	int fd;
	int ret;
	...
	if (champ->header.prog_name[PROG_NAME_LENGTH] || champ->header.how[COMMENT_LENGTH])
...
int get_nb_champs(int ac, char **av)
{
	...
	ret = 0;
	while (++n < ac)
		if (av[n][0] != '-' && ft_strcmp(av[n - 1], "-d") && ft_strcmp(av[n - 1], "-dump"))
			ret++;
	return (ret);
}
...
t_champ_decoded *parser(char **players, int nb_champs, uint32_t *num_champs)
{
	...
	n = -1;
	champs_decoded = NULL;
	if (!(champs_decoded = ft_memalloc(sizeof(t_champ_decoded) * (nb_champs + 1))))
