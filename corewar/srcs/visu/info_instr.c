...
static int trasform_info(char **info, t_instr_v instr, char *tmp)
{
	if (!(info[0] = ft_strdup("instruction")) ||
		...
		return (0);
	tmp = instr.ocp != 255 ? ft_ocptoa(instr.ocp, instr.nb_args) : ft_strdup("none");
	if (!(info[2] = tmp))
		return (0);
...
static int print_info_mem2(char ***info, SDL_Rect *coord, t_instr_v instr)
{
	...
	tmp = NULL;
	if (!(*info = ft_memalloc(sizeof(char *) * (7))) || !trasform_info(*info, instr, tmp))
	{
		free_info(*info);
		return (0);
	}
...
int print_info_mem(t_vm *vm, SDL_Rect coord, t_instr_v instr)
{
	...
	{
		info_coord = coord;
		SDL_BlitSurface(vm->visu.command_text[n], 0, vm->visu.window_surface, &info_coord);
		info_coord = coord;
		info_coord.x += coord.w / 3;
		if (!(text = TTF_RenderText_Blended(
				  vm->visu.info_font, info[n], vm->visu.text_color)))
			return (0);
