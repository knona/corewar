...
static void print_number(t_vm *vm, int n, int m)
{
	...
	{
		start_x = vm->visu.memory_coord.x + (n % m) * (vm->visu.mem_len.w) +
			vm->visu.mem_len.w * 0.1;
		...
		SDL_BlitSurface(
			vm->visu.hex[vm->arena.mem[n] / 16], NULL, vm->visu.window_surface, &coord);
...
static void print_champ(t_vm *vm, SDL_Rect coord, int n)
{
	...
	{
		coord.x +=
			vm->visu.mem_len.w / 2 - vm->visu.champ[vm->arena.champs[n] - 1]->w / 2;
		coord.y +=
			vm->visu.mem_len.h / 2 - vm->visu.champ[vm->arena.champs[n] - 1]->h / 2;
		SDL_BlitSurface(
			vm->visu.champ[vm->arena.champs[n] - 1], 0, vm->visu.window_surface, &coord);
	}
	else if (vm->arena.processes[n] < 100)
	{
		...
		printed.w = vm->visu.process->w -
			(((vm->visu.mem_len.w * 0.8) * vm->arena.processes[n]) / 100);
		if (vm->arena.processes[n])
			printed.w -= (vm->visu.process->w - vm->visu.mem_len.w * 0.8) / 2;
		printed.h = vm->visu.process->h;
		SDL_BlitSurface(vm->visu.process, &printed, vm->visu.window_surface, &coord);
	}
}
