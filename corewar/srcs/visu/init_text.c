...
int init_info(t_vm *vm)
{
	...
	tmp = 0;
	if (!(tmp = ft_strjoin_mult(4, vm->visu.path, ASSET_PATH, FONT_PATH, FONT)) ||
		!(vm->visu.info_font = TTF_OpenFont(tmp, ft_min(w / 10, h / 10))))
		return (0);
	free(tmp);
...
int init_champ_name(t_vm *vm)
{
	...
	while (++n < vm->nb_champs)
	{
		...
		name = TTF_RenderText_Blended(vm->visu.info_font, header.prog_name, color);
		if (!name)
			name = TTF_RenderText_Blended(vm->visu.info_font, "NO NAME", color);
...
static int init_info_text2(t_vm *vm,
						   char info_text[8][14],
						   char command_text[6][6],
						   char process_text[3][8][8])
{
	int n;
	...
	{
		if (n < 4 &&
			!(vm->visu.info_text[n] = TTF_RenderText_Blended(
				  vm->visu.info_font, info_text[n], vm->visu.text_color)))
			return (0);
		if (n < 6 &&
			!(vm->visu.command_text[n] = TTF_RenderText_Blended(
				  vm->visu.info_font, command_text[n], vm->visu.text_color)))
			return (0);
		if (n != 7 &&
			!(vm->visu.process_text[n / 8][n % 8] = TTF_RenderText_Blended(
				  vm->visu.info_font, process_text[n / 8][n % 8], vm->visu.text_color)))
			return (0);
	}
	return (1);
}
...
{
	static char info_text[8][14] = { "CYCLE", "PROCESS", "CYCLE TO DIE", "SPEED" };
	static char command_text[6][6] = { "type:", "name:", "ocp:",
									   "arg1:", "arg2:", "arg3:" };
