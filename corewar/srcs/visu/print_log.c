...
static int fill_info(t_vm *vm, char **info)
{
	...
	if (!(info[0] = ft_itoa(vm->cycle)) || !(info[1] = ft_itoa(vm->nb_procs)) ||
...
int print_info_log_text(t_vm *vm, char *info, int n)
{
	...
	coord.y = vm->visu.log_coord.y + ((vm->visu.log_coord.h * 15) / 20) +
		n * (6 * vm->visu.little_info->h) / 5 + vm->visu.little_info->h / 2 -
	...
	if (!(text = TTF_RenderText_Blended(vm->visu.info_font, info, vm->visu.text_color)))
		return (0);
	coord.y = vm->visu.log_coord.y + ((vm->visu.log_coord.h * 15) / 20) +
		n * (6 * vm->visu.little_info->h) / 5;
	coord.x =
		vm->visu.log_coord.x + vm->visu.log_coord.w / 2 + vm->visu.log_coord.w / 50;
