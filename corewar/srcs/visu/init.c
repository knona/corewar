...
void init_coord(t_vm *vm, int win_width, int win_height)
{
	...
	vm->visu.win_len.h -=
		(vm->visu.win_len.h - vm->visu.memory_coord.y * 2 - 7) % ft_sqrt(MEM_SIZE);
	vm->visu.log_coord.h = vm->visu.win_len.h;
	vm->visu.log_coord.w = (vm->visu.win_len.h * FRAME_WIDTH) / FRAME_LEN;
	vm->visu.memory_coord.w =
		vm->visu.win_len.w - vm->visu.log_coord.w - vm->visu.memory_coord.x * 2;
	vm->visu.memory_coord.w -= vm->visu.memory_coord.w % ft_sqrt(MEM_SIZE);
	vm->visu.memory_coord.h = vm->visu.win_len.h - 7 - vm->visu.memory_coord.y * 2;
	vm->visu.arena_coord.w = vm->visu.memory_coord.w + vm->visu.memory_coord.x * 2;
...
{
	if (choice == 0)
		return (!(vm->visu.window = SDL_CreateWindow("CoreWar",
		...
		return (!(vm->visu.window_surface = SDL_GetWindowSurface(vm->visu.window)) ||
				!init_text(vm) || !init_info(vm) || !load_images(vm));
	else if (choice == 2)
		return (!(vm->visu.arena = SDL_CreateRGBSurface(
					  0, vm->visu.arena_coord.w, vm->visu.arena_coord.h, 32, 0, 0, 0, 0)) ||
				!(vm->visu.log = SDL_CreateRGBSurface(
					  0, vm->visu.log_coord.w, vm->visu.log_coord.h, 32, 0, 0, 0, 0)));
	else
		return (0);
}
