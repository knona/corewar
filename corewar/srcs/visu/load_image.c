...
static int imgs_load(t_vm *vm, char *arr_img[4], SDL_Surface **surface)
{
	...
	while (i < 4)
	{
		if (!(tmp = ft_strjoin_mult(4, vm->visu.path, ASSET_PATH, IMAGE_PATH, arr_img[i])) ||
			!(surface[i] = IMG_Load(tmp)))
			return (0);
		free(tmp);
		i++;
	}
...
static int init_champ_jauge(t_vm *vm)
{
	...
	if (!imgs_load(vm, arr_img, vm->visu.champ_jauge_halo_src))
		return (0);
	SDL_SetSurfaceBlendMode(vm->visu.champ_jauge_halo_src[0], SDL_BLENDMODE_NONE);
	SDL_SetSurfaceBlendMode(vm->visu.champ_jauge_halo_src[1], SDL_BLENDMODE_NONE);
	SDL_SetSurfaceBlendMode(vm->visu.champ_jauge_halo_src[2], SDL_BLENDMODE_NONE);
	SDL_SetSurfaceBlendMode(vm->visu.champ_jauge_halo_src[3], SDL_BLENDMODE_NONE);
	return (1);
}
...
static int init_champ(t_vm *vm)
{
	...
	if (!(tmp = ft_strjoin_mult(4, vm->visu.path, ASSET_PATH, IMAGE_PATH, "Square0_Halo.png")) ||
...
int load_images(t_vm *vm)
{
	...
	if (!(tmp = ft_strjoin_mult(4, vm->visu.path, ASSET_PATH, IMAGE_PATH, "frame.png")))
		return (0);
	if (!(vm->visu.log_background = IMG_Load(tmp)))
		return (0);
	free(tmp);
	if (!(tmp = ft_strjoin_mult(4, vm->visu.path, ASSET_PATH, IMAGE_PATH, "Area.png")))
		return (0);
