...
static int treat_info_process(t_vm *vm, char **info, t_process process)
{
	...
	if (!(info[0] = ft_strdup("process")) || !(info[1] = ft_itoa(process.num)) ||
		!(info[2] = get_champ_name(vm, process.visu_id, 10)) ||
		!(info[3] = ft_itoa(process.carry)) || !(info[6] = ft_itoa(process.cycles_left)))
		return (0);
...
static void print_info_process2(SDL_Rect *info_coord, SDL_Rect coord, int n, t_vm *vm)
{
	*info_coord = coord;
	SDL_BlitSurface(
		vm->visu.process_text[vm->visu.info_pages - 1][n], 0, vm->visu.window_surface, info_coord);
	*info_coord = coord;
...
int print_info_process(t_vm *vm, SDL_Rect coord, t_process process, int change)
{
	...
	if (change == 2 && free_info_process(&info))
		return (1);
	if (change && free_info_process(&info) &&
		(!(info = ft_memalloc(sizeof(char *) * (25))) || !treat_info_process(vm, info, process)))
		return (0);
	coord.x += coord.w / 20;
	coord.y += coord.h / 20;
	n = -1;
