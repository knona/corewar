...
int rev_image(SDL_Surface **new, SDL_Surface *ref)
{
	...
	if (!(*new = SDL_CreateRGBSurfaceWithFormat(
			  0, ref->h, ref->w, 32, ref->format->format)))
		return (0);
	SDL_LockSurface(*new);
	...
	while (++y < ref->h)
	{
		x = -1;
		while (++x < ref->w)
			pixels[ref->h * ((ref->w - 1) - x) + y] = pixels_ref[y * ref->w + x];
	}
...
SDL_Surface *create_empty_frame(t_len frame_len, int frame_color, int len)
{
	...
	if (!(frame = SDL_CreateRGBSurface(0, frame_len.w, frame_len.h, 32, 0, 0, 0, 0)))
		return (0);
...
SDL_Surface *create_filled_frame(t_len frame_len, int frame_color, int background_color, int len)
{
	SDL_Surface *frame;
	SDL_Rect coord;
...
void get_width(t_vm *vm, int *w, int *h, int choice)
{
	...
	surface = !choice ? vm->visu.champ_jauge_src : vm->visu.champ_jauge_halo_src;
