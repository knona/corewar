...
static void print_jauge_halo(t_vm *vm, SDL_Rect coord, SDL_Surface *halo, int len)
{
	SDL_Rect printed;
...
static void print_jauge2(SDL_Rect *coord, int x, int n, t_vm *vm)
{
	...
	coord->x = vm->visu.log_coord.x + x * n + (vm->visu.log_coord.w / 10) -
		(vm->visu.champ_jauge[0]->w + 2) / 2 - 1 - vm->visu.champ_name[n - 1]->w;
	coord->y = vm->visu.log_coord.y + ((vm->visu.log_coord.h * 13) / 20) -
		vm->visu.champ_name[n - 1]->h;
	SDL_BlitSurface(vm->visu.champ_name[n - 1], 0, vm->visu.window_surface, coord);
	coord->w = vm->visu.champ_jauge[0]->w;
	coord->h = get_jauge_height(vm, vm->visu.champ_jauge[n - 1]->h, n - 1);
	coord->x =
		vm->visu.log_coord.x + x * n + (vm->visu.log_coord.w / 10) - coord->w / 2;
	coord->y = vm->visu.log_coord.y + ((vm->visu.log_coord.h * 13) / 20) - coord->h;
	SDL_BlitScaled(vm->visu.champ_jauge[n - 1], 0, vm->visu.window_surface, coord);
	coord->x = vm->visu.log_coord.x + x * n + (vm->visu.log_coord.w / 10);
	coord->y = vm->visu.log_coord.y + ((vm->visu.log_coord.h * 13) / 20);
