...
static void print_process_3(t_process **process, int *process_num, int *old_mem, t_vm *vm)
{
	(*process_num)++;
...
static void print_process_4(t_process **process, int *process_num, int *old_mem, t_vm *vm)
{
	(*process_num)--;
	if (*process_num < 0)
...
int print_process(t_vm *vm, int mem, int status)
{
	...
	coord = treat_coord(vm,
						(mem % r) * vm->visu.mem_len.w + vm->visu.memory_coord.x,
						(mem / r) * vm->visu.mem_len.h + vm->visu.memory_coord.y,
						vm->visu.info_pages == 1 ? 7 : 8);
