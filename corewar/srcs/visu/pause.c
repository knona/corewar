...
static int pause_visu3(t_vm *vm, SDL_Event event)
{
	if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_KP_PLUS &&
		vm->visu.speed < 64)
	{
		vm->visu.speed = vm->visu.speed < 1 ? vm->visu.speed / 2 : vm->visu.speed * 2;
		if (vm->visu.speed == -1)
			vm->visu.speed = 1;
		print_log(vm);
		SDL_UpdateWindowSurface(vm->visu.window);
			 ...
			 event.key.keysym.sym == SDLK_KP_MINUS && vm->visu.speed > -64)
	{
		if (vm->visu.speed < 1)
			vm->visu.speed *= 2;
		else
...
{
	if (pause_visu3(vm, event) && event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_r)
	{
		vm->visu.speed = 1;
		print_log(vm);
		SDL_UpdateWindowSurface(vm->visu.window);
	...
	if (event.wheel.type == SDL_MOUSEWHEEL)
	{
		...
		if (mem > 0 && mem < MEM_SIZE && mouse.x >= 0 &&
			mouse.x < vm->visu.memory_coord.w && mouse.y > 0 &&
			vm->arena.processes[mem] <= 100)
		{
			if (event.wheel.y > 0)
