...
{
	color[2] =
		color_max(((CYCLE2 & 0xFF0000) >> 16) * n / (vm->visu.memory_coord.w / 2),
				  (CYCLE1 & 0xFF0000) >> 16);
	color[1] = color_max(((CYCLE2 & 0xFF00) >> 8) * n / (vm->visu.memory_coord.w / 2),
						 (CYCLE1 & 0xFF00) >> 8);
	color[0] = color_max((CYCLE2 & 0xFF) * n / (vm->visu.memory_coord.w / 2), CYCLE1 & 0xFF);
}
...
static void init_cycle3(t_vm *vm, unsigned char *color, int n)
{
	...
	color[2] =
		color_max(((CYCLE1 & 0xFF0000) >> 16) * tmp / (vm->visu.memory_coord.w / 2),
				  (CYCLE2 & 0xFF0000) >> 16);
	color[1] = color_max(((CYCLE1 & 0xFF00) >> 8) * tmp / (vm->visu.memory_coord.w / 2),
						 (CYCLE2 & 0xFF00) >> 8);
	color[0] = color_max((CYCLE1 & 0xFF) * tmp / (vm->visu.memory_coord.w / 2), CYCLE2 & 0xFF);
}
