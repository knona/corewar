...
void play_music(t_vm *vm)
{
	...
	{
		if (!(tmp = get_groove(ft_strjoin_mult(4, vm->visu.path, ASSET_PATH, SOUND_PATH, "kitch/"))))
			return;
	}
	else if (!(tmp = get_groove(ft_strjoin_mult(
				   4, vm->visu.path, ASSET_PATH, SOUND_PATH, "non_kitch/"))))
		return;
