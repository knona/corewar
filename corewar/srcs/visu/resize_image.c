...
static void resize_all_images2(t_vm *vm, SDL_Rect *rect, int w, int h)
{
	...
	rect->y = h / 2 - rect->h / 2;
	SDL_FillRect(
		vm->visu.process, rect, SDL_MapRGB(vm->visu.process->format, 0xFF, 0xFF, 0xFF));
	SDL_FillRect(
		vm->visu.champ[0], rect, SDL_MapRGB(vm->visu.champ[0]->format, 0xFF, 0x00, 0x3C));
	SDL_FillRect(
		vm->visu.champ[1], rect, SDL_MapRGB(vm->visu.champ[1]->format, 0xFF, 0xFF, 0x39));
	SDL_FillRect(
		vm->visu.champ[2], rect, SDL_MapRGB(vm->visu.champ[2]->format, 0x59, 0xFF, 0x63));
	SDL_FillRect(
		vm->visu.champ[3], rect, SDL_MapRGB(vm->visu.champ[3]->format, 0x6b, 0x68, 0xFF));
	rect->w = vm->visu.mem_len.w;
	rect->x = w / 2 - rect->w / 2;
	SDL_FillRect(
		vm->visu.highlight, rect, SDL_MapRGB(vm->visu.process->format, 0xFF, 0xFF, 0xFF));
}

static int resize_all_images_if(t_vm *vm, int w, int h)
{
	return (!(vm->visu.process = SDL_CreateRGBSurfaceWithFormat(
...
{
	return (
		!(vm->visu.champ_jauge[0] = SDL_CreateRGBSurface(0, w, h, 32, 0, 0, 0, 0)) ||
		!(vm->visu.champ_jauge[1] = SDL_CreateRGBSurface(0, w, h, 32, 0, 0, 0, 0)) ||
		!(vm->visu.champ_jauge[2] = SDL_CreateRGBSurface(0, w, h, 32, 0, 0, 0, 0)) ||
		!(vm->visu.champ_jauge[3] = SDL_CreateRGBSurface(0, w, h, 32, 0, 0, 0, 0)));
}

static int resize_all_images_if3(t_vm *vm, int w, int h)
{
	return (!(vm->visu.champ_jauge_halo[0] = SDL_CreateRGBSurfaceWithFormat(
...
int resize_all_images(t_vm *vm)
{
	...
	i = -1;
	while (++i < 4)
		SDL_BlitScaled(vm->visu.champ_jauge_src[i], 0, vm->visu.champ_jauge[i], 0);
	get_width(vm, &w, &h, 1);
	...
	while (++i < 4)
		SDL_BlitScaled(
			vm->visu.champ_jauge_halo_src[i], 0, vm->visu.champ_jauge_halo[i], 0);
	return (1);
}
