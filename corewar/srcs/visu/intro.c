...
static void get_shade(SDL_Surface *ref_image, SDL_Surface *image_shaded, int shade)
{
	SDL_Color *pixel;
	int n;
...
static int launch_intro2(t_vm *vm, SDL_Surface **intro_image, SDL_Surface **shaded_image, SDL_Rect *coord)
{
	char *tmp;

	tmp = ft_strjoin_mult(4, vm->visu.path, ASSET_PATH, IMAGE_PATH, "IntroCoreWar.png");
	*intro_image = IMG_Load(tmp);
	SDL_SetSurfaceBlendMode(*intro_image, SDL_BLENDMODE_NONE);
	if (!(*shaded_image = SDL_CreateRGBSurface(
			  0, (*intro_image)->w, (*intro_image)->h, 32, 0, 0, 0, 0)))
		return (0);
...
static int intro_event(t_vm *vm, SDL_Surface *intro_image, SDL_Surface *shaded_image, SDL_Rect *coord)
{
	SDL_Event event;
	...
	while (SDL_PollEvent(&event))
	{
		...
		else if (event.window.event == SDL_WINDOWEVENT_RESIZED)
		{
			...
			SDL_SetWindowSize(
				vm->visu.window, vm->visu.win_len.w, vm->visu.win_len.h);
			*coord = get_intro_coord(vm, intro_image);
		}
...
void launch_intro(t_vm *vm)
{
	...
	while (1)
	{
		if (intro_event(vm, intro_image, shaded_image, &coord) || SDL_GetTicks() > 4000)
		{
			SDL_FreeSurface(intro_image);
			SDL_FreeSurface(shaded_image);
			return;
