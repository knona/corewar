...
void execute_instr2(t_vm *vm, t_process *process, t_instr_ret *instr_ret, int *size_instr)
{
	int index_op;
	int size_args;
	...
	if (is_instr_without_ocp(process->op))
	{
		*size_instr = g_op_tab[index_op].dir_size + 1;
		g_instrs_f[process->op - 1](process, &vm->arena, instr_ret, vm->options.verbose);
		process->pc = (process->pc + *size_instr) % MEM_SIZE;
	}
	else
	{
		size_args = get_size_args(*(vm->arena.mem + (process->pc + 1) % MEM_SIZE), index_op);
		*size_instr = (size_args < 0 ? -size_args : size_args) + 1 + 1;
		if (size_args > 0)
			g_instrs_f[process->op - 1](
				process, &vm->arena, instr_ret, vm->options.verbose);
		process->pc = (process->pc + *size_instr) % MEM_SIZE;
	}
}
