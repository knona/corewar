...
{
	if (process->op != -1)
		vm->arena.processes[process->pc] =
			(process->cycles_left / (double)g_op_tab[search_op(process->op)].nb_cycles) * 100;
	else
...
int corewar_loop(t_vm *vm, int *last_cycle)
{
	...
	check_process = vm->cycle_to_die <= 0 ||
		(vm->cycle - *last_cycle) % vm->cycle_to_die == 0;
...
int corewar(t_vm *vm)
{
	...
	while (vm->list)
	{
		if (vm->options.cycle_dump != -1 && vm->options.cycle_dump == vm->cycle - 1)
			return (memdump(vm));
