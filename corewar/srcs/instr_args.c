...
{
	memload(vars->args, ar->mem, g_op_tab[vars->index_op].dir_size, get_mempos(pro->pc + 1));
...
static int transform(t_process *pro, t_arena *ar, t_instr_vars *vars, int i)
{
	...
	else if (nb == IND_CODE)
	{
		temp = vars->args[i];
		mem_swap(&temp, 2);
		memload(&vars->f_args[i],
				ar->mem,
				vars->ind_size,
				get_mempos(pro->pc + (vars->idx ? (int16_t)temp % vars->idx : (int16_t)temp)));
	}
	else
