...
static int ft_arg_handler_bis(char *flags, t_fnct *fnct, int *flist, int fd)
{
	...
	else if (flags[(int)flags[0]] == '!')
	{
		...
		size = fnct[(int)flags[(int)flags[0]]](
			flist, (va_list *)&flags[(int)flags[0]], fd);
	free(flist);
	free(flags);
	if (size < 0)
		return (-1);
...
static int ft_arg_handler(char *flags, va_list *args, int fd, int i)
{
	...
	if (fnct[(int)flags[len - 1]] != fnct[0] && flags[len - 1] != '@' && flags[len - 1] != '!')
...
int ft_parser(const char *restrict str, va_list *args, int fd)
{
	...
	while (end != 0 && end != -1)
	{
		...
		if (start != -1)
		{
			...
			if (end != -1)
				checksize(&size, ft_arg_handler(ft_strsub(str, start, end + 1), args, fd, 0));
